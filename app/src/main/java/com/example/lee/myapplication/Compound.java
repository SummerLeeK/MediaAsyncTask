...
public class Compound {
    public static int MP3_TYPE = 0;
    public static int AAC_TYPE = 1;
    public static int M4A_TYPE = 2;
    public static int NOT_DC_EC_TYPE = 3;//不用重新编解码
    public static int NOT_SUPPORT_TYPE = 4;//不支持类型
     ...
     */
    private Compound(String videoPath, String audioPath, int audioDEType, String dstPath) {
     ...
     */
    public static Compound createCompounder(String videoPath, String audioPath, String dstPath) {
        if (checkVideo(videoPath)) {
            int audioEDType = checkAudio(audioPath);
            if (audioEDType != NOT_SUPPORT_TYPE)
                return new Compound(videoPath, audioPath, audioEDType, dstPath);
        }
        return null;
    }
